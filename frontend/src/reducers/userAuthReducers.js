...
try {
  authData = localStorage.getItem("userInfo")
    ? { ...authData, user:  decodeData(localStorage.getItem("userInfo")), message:"Main-try-true" }
...
export const authReducer = (user = authData, action) => {
  switch (action.type) {
    ...
    console.log("Here inside auth");
      ...
      return { ...user, user:action.payload ,message:"signup" };

    case USER_SIGNIN:
      localStorage.setItem("userInfo", encodeData(action.payload));
      return { ...user, user:action.payload , message:"signin"};

    case USER_VERIFIED:
      localStorage.setItem("userInfo", encodeData(action.payload));
      return { ...user, user:action.payload , message:"varify"};

    case USER_SIGNOUT:
      localStorage.removeItem('userInfo')
      return { ...user, user:null,message:"Signout" };
