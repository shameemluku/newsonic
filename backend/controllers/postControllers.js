...
const postComment = async (req, res) => {
  ...
  let response = await Posts.findByIdAndUpdate({_id:ObjectId(postId)},{$push:{comments:commentData}})
...
const likePost = async (req, res) => {
  ...
  if(!userId){
    ...
    let response = await Posts.findOne({_id:ObjectId(postId),likes:ObjectId(userId)})

    if(response){
    
      let like_result = await Posts.updateOne({_id:Object(postId)},{ $pull:{likes:ObjectId(userId)} })
    
      res.status(200).json({status:false,message:"You Unliked the post"})
    }else{
    
      let like_result = await Posts.updateOne({_id:Object(postId)},{$addToSet:{likes:ObjectId(userId)}})
...
const savePost = async (req, res) => {
  ...
  if(!userId){
    ...
    let response = await User.findOne({_id:ObjectId(userId),saved:ObjectId(postId)})

    if(response){
    
      let save_result = await User.updateOne({_id:Object(userId)},{ $pull:{saved:ObjectId(postId)} })
    
      res.status(200).json({status:false,message:"You Saved the post"})
    }else{
    
      let save_result = await User.updateOne({_id:Object(userId)},{$addToSet:{saved:ObjectId(postId)}})
...
const deletePost = async (req, res) => {
  ...
  if(response){
    res.status(200).json({status:true,message:"Documents deleted successfully!"})
  }
...
const relatedPost = async (req, res) => {
  ...
  let response = await Posts.find({$in:{category:req.body}}).sort({_id:-1})
